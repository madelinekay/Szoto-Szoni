...
public interface RootCRepository extends JpaRepository<Root, Integer> {
    ...
    @Query(value = "select *, wr.mutation from rootc r inner join root_word wr on r.id=wr.rootc_id where wr.wordc_id=:wordId order by wr.position", nativeQuery = true)
